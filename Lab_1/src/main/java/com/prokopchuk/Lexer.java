...
public class Lexer {
  ...
  public List<TokenWrapper> tokenize(String fileName) {
  ...
  private void processSymbol(Character c) {
    ...
    switch (currentState) {
      ...
      case GREATER -> greaterState(c);
      case LESS -> lessState(c);
      case AND -> andState(c);
      case OR -> orState(c);
      case OPERATOR -> operatorState(c);
      case COLON -> colonState(c);
      case PLUS -> plusState(c);
      case MINUS -> minusState(c);
  ...
  private void startState(Character c) {
    if (c == '/') {
      ...
      log.error("Can't find state for symbols {}", stateBuffer.toString());
    }
  }
